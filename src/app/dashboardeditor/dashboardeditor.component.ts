...
export class DashboardeditorComponent implements OnInit {
  ...
  loadProp($event: any, data: any) {
    ...
    console.log($event.path.find(function (element) {
      return element.localName == "mat-grid-tile";
    }));
...
export class gridselectionDialog {
  ...
  private _mdialog;
  constructor(public dialog: MatDialogRef<gridselectionDialog>,
    private _dataStoreService: DatastoreService) {
    //super(dialog);
...
export class DialogCommon {
  ...
  constructor(
    public dialogCom: MatDialogRef<DialogCommon>, @Inject(MAT_DIALOG_DATA) public data: any) { }
